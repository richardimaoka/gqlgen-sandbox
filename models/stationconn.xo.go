...
func StationConnsByStationCD(ctx context.Context, db DB, stationCD int) ([]*StationConn, error) {
	...
	const sqlstr = `select li.line_name, ` +
		...
		`left outer join joins sjb on st.line_cd = sjb.line_cd and st.station_cd = sjb.station_cd2 ` +
		`left outer join joins sja on st.line_cd = sja.line_cd and st.station_cd = sja.station_cd1 ` +
